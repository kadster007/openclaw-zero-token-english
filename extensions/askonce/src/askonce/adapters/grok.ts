...
import { createGrokWebStreamFn } from "../../../../../src/zero-token/streams/grok-web-stream.js";
...
export class GrokAdapter extends BaseAdapter {
  ...
  private async getCredential(): Promise<string | null> {
    ...
    try {
      ...
      const profiles = listProfilesForProvider(store, "grok-web");
  ...
  async query(question: string, options?: AdapterQueryOptions): Promise<ModelResponse> {
    ...
    try {
      ...
      const streamFn = createGrokWebStreamFn(credential);

...
import { createGeminiWebStreamFn } from "../../../../../src/zero-token/streams/gemini-web-stream.js";
...
export class GeminiAdapter extends BaseAdapter {
  ...
  private async getCredential(): Promise<string | null> {
    ...
    try {
      ...
      const profiles = listProfilesForProvider(store, "gemini-web");
  ...
  async query(question: string, options?: AdapterQueryOptions): Promise<ModelResponse> {
    ...
    try {
      ...
      const streamFn = createGeminiWebStreamFn(credential);

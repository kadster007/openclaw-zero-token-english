...
export async function loginDeepseekWebAttachOnly(params: {
  ...
  try {
    ...
    // Check if there is already an open DeepSeek page
    ...
    if (!page) {
      ...
      // Existing DeepSeek page found, switch to it
...
export async function loginDeepseekWeb(params: {
  ...
  try {
    ...
    // If already logged in, try to get bearer token directly
    if (hasValidSession) {
      ...
      // API returned no token (may be expired), clear session state and let user re-login
      params.onProgress("Session detected but token expired. Redirecting to login page...");
      hasValidSession = false;
    }

    // Not logged in or unable to auto-capture, navigate to login page and wait for user login
    await page.goto("https://chat.deepseek.com");
    userAgent = await page.evaluate(() => navigator.userAgent);
